...
// Empty JSON content for disabling particles
...
pub(crate) unsafe fn open(
    ...
    if is_particle_json_file(c_path) {
        log::info!("Intercepting particle JSON file with empty content: {}", c_path.display());
        let buffer = EMPTY_JSON.as_bytes().to_vec();
        let mut wanted_lock = WANTED_ASSETS.lock().unwrap();
        wanted_lock.insert(AAssetPtr(aasset), Cursor::new(buffer));
        return aasset;
    }
